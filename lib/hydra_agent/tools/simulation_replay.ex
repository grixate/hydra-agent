defmodule HydraAgent.Tools.SimulationReplay do
  ...
  def spec do
    %{
      name: "simulation_replay",
      side_effect_class: "read_only",
      timeout_ms: 10_000,
      approval_sensitive: false,
      description: "Read a filtered replay for a workspace simulation.",
  ...
  def execute(input, context) do
    ...
    with {:ok, id} <- required_id(input["simulation_id"]),
         {:ok, simulation} <- Simulation.fetch_simulation_for_workspace(workspace_id, id) do
      {:ok, Simulation.replay(simulation, input)}
    else
