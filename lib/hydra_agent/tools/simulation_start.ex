defmodule HydraAgent.Tools.SimulationStart do
  ...
  def spec do
    %{
      ...
      description: "Start an existing workspace simulation asynchronously.",
  ...
  def execute(input, context) do
    ...
    with {:ok, id} <- required_id(input["simulation_id"]),
         ...
         {:ok, started} <- Simulation.start_simulation(simulation) do
      {:ok, %{"simulation" => Simulation.simulation_json(started)}}
    else
